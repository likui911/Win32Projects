	...
	PSTR szCmdLine, int iCmdShow)
{
	...
	wndclass.hIcon = (HICON)LoadImage(hInstance,
		MAKEINTRESOURCE(IDI_ICON), IMAGE_ICON, SM_CXICON, SM_CYICON, FALSE);
	//wndclass.hCursor = LoadCursor(NULL, IDC_ARROW);
	wndclass.hCursor = (HCURSOR)LoadImage(NULL, IDC_ARROW,
		IMAGE_CURSOR, SM_CXCURSOR, SM_CYCURSOR, FALSE);
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hResource = LoadResource((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE),
			FindResource((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE), MAKEINTRESOURCE(IDR_TEXT),
				TEXT("TEXT")));
		...
		//*pText = '\0';//write access violation
	...
	case WM_MOUSEMOVE:
		SetCursor((HCURSOR)LoadImage((HINSTANCE)GetWindowLongPtr(hwnd, GWLP_HINSTANCE),
			MAKEINTRESOURCE(IDC_CURSOR), IMAGE_CURSOR, SM_CXCURSOR * 2, SM_CYCURSOR * 2, FALSE));
		return 0;
